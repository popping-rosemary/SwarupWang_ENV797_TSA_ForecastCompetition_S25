...
K_yearly = 4
for_period = 59
NN_fits <- function(ts, p, P, K_weekly, K_yearly) {
  # Generate Fourier terms for both weekly and yearly seasonality
  n <- length(ts)
  t <- 1:n
  weekly_fourier <- matrix(0, nrow = n, ncol = 2 * K_weekly)
  for (i in 1:K_weekly) {
    weekly_fourier[, 2 * i - 1] <- sin(2 * pi * i * t / 7)
    weekly_fourier[, 2 * i] <- cos(2 * pi * i * t / 7)
  }
  yearly_fourier <- fourier(ts, K = K_yearly)

  # Combine the Fourier terms into the xreg matrix
  xreg_vars <- cbind(ts, weekly_fourier, yearly_fourier)
  
  model_fit <- nnetar(ts,
                      p = p,
                      P = P,
                      xreg = fourier(xreg_vars))
  return(model_fit)
}
model_fit_1 <- NN_fits(load_ts, p = 4, P = 1, K_weekly = 2, K_yearly = 4)
residuals_fit <- residuals(model_fit_1)

acf(residuals_fit[!is.na(residuals_fit)], main = "ACF of Residuals")
pacf(residuals_fit[!is.na(residuals_fit)], main = "PACF of Residuals")
```


```{r}
#Continue tweaking the NN model
K = 2
for_period = 59
NN_forecast <- function(ts, p, P, K) {
  model_fit <- nnetar(ts,
...
forecast_2 <- NN_forecast(load_ts, 4, 0, K)
